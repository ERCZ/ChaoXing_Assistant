// ==UserScript==
// @name         超星尔雅自动刷课助手
// @namespace    xyz-studio.xyz
// @version      1.0.0
// @description  [本脚本由原作者wyn665817的超星网课助手3.3.1版本修改而来]自动挂机看尔雅MOOC，支持视频、音频、文档、图书自动完成，章节测验自动答题提交，支持自动切换任务点、挂机阅读时长、自动登录等，解除各类功能限制，开放自定义参数
// @author       xyz-studio
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
...
function beforeFind() {
    ...
    if (parent._data === 0) return setting.regl();
    setting.id = $('#workLibraryId').val() || $('#oldWorkId').val();
    setting.div = $(
    ...
    setting.lose = setting.num = parent._data = 0;
            ...
            if (!setting.loop) {
                ...
                if (obj.code) {
                    ...
                    var td = '<td style="border: 1px solid;',
                        data = obj.answer.replace(/&/g, '&amp;').replace(/<(?!img)/g, '&lt;');
                    obj.data = /^http/.test(data) ? '<img src="' + obj.answer + '">' : obj.answer;
                    $(
                        '<tr>' +
