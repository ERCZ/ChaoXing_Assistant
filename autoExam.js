...
// @version      1.0.0
// @description  [本脚本由原作者wyn665817的超星网课助手(考试专版)2.4.0版本修改而来]自动搜索尔雅MOOC考试答案，支持自动答题、自动切换题目、隐藏答案搜索提示框等，解除各类功能限制，开放自定义参数
// @author       xyz-studio
                ...
                if (obj.code) {
                    ...
                    setting.div.find('tbody').append(
                        '<tr>' +
                            ...
                            '<td title="点击可复制">' + (/^http/.test(data) ? '<img src="' + obj.answer + '">' : '') + data + '</td>' +
                        '</tr>'
                    );
                    ...
                    setting.$div.html('服务器繁忙，正在重试...');
                }
                setting.div.children('span').html(obj.answer || '');
...
function fillAnswer(obj, tip) {
    ...
    str = String(obj.answer).trim().toCDB(),
